...
- **상위 유사도 top k 를 추출할 경우 k 값이 일정 수준 이상일 경우 근사 성능이 감소합니다. (500 정도로 알려짐)**
...
- **즉 비교적 높은 차원의 벡터를 다루면서 많은 top k가 필요하거나 대상 벡터의 숫자가 100만 미만으로 충분히 작다면 Brute Force 계산을 하여도 괜찮습니다.**
...
- **테스트에서 모든 recall은 실제로 1로 계산 되었지만 GPU 연산의 비 결정성과 cublas의 MatmulAlgoGetHeuristic의 동작에 의한 Dynamic Batch 상황시 tensor shape 가변에 따른 따른 묵시적인 GEMM 알고리즘 변경으로 수치적 오차가 생길 가능성이 있습니다.**
